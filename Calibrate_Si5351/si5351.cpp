 ...
 *
 *	Original By: T.Uebo (JF3HZB) 02/11/2019
 *
 *	Modified by: John M. Price (WA2FZW) 05/09/19
 *
 *		General cleanup, formatting and adding comments.
 *		The I2C address is now defined in "config.h".
 *		Moved some instruction sequences that were common to "Set_VFO_Freq"
 *			and "Set_Carrier_Freq" into common functions.
 *
 *
 *	Modified by John Price (WA2FZW) - 05/26/19 Version 5.4
 *
 *		Added support for the PCF8574 GPIO extender chip as an alternate means
 *		of reading the band switch. This required putting the Si5351 on its
 *		own I2C bus seperate from the standard one due to some kind of conflict
 *		in the "si5351.cpp" module. The SDA & SCL pins for the Si5351 are now
 *		defined in "config.h" as "SI_SDA" and "SI_SCL".
 */

#include <arduino.h>						// General Arduino definitions
#include "Cal_Config.h"						// Configuration definitions
#include "si5351.h"							// Si5351 stuff

volatile uint32_t oMf = 0;
volatile uint32_t oMc = 0;

static	 uint32_t xFreq = SI_XTAL;			// Default to setting in "Cal_Config.h"
 ...
 */
	...
	if ( MODE == C_OSC_CLK0 )						// CLK0 Only?
	{
		cmd_si5351 ( 17, 0x80 );					// Turn off CLK1
		return;										// Done
	}

	if ( MODE == C_OSC_CLK1 )						// CLK1 Only?
	{
		cmd_si5351 ( 16, 0x80 );					// Turn off CLK0
		return;										// Done
	}
